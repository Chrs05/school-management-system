...
class StudentRoleController extends Controller
{
    //
     ...
     public function StudentRegRole(){

        $data['years'] = StudentYear::all();
    	$data['classes'] = StudentClass::all();

        return view('backend.student.role_generate.student_role_view', $data);
    ...
    public function StudentRoleStore(Request $request){
    	...
    	if ($request->student_id !=null) {
    		for ($i=0; $i < count($request->student_id); $i++) {
    			AssignStudent::where('year_id',$year_id)->where('class_id',$class_id)->where('student_id',$request->student_id[$i])->update(['role' => $request->role[$i]]);
    		} // end for loop
    	} else {
    		$notification = array(
